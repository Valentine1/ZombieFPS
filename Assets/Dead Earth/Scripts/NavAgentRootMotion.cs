...
public class NavAgentRootMotion : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		if ( _navAgent.desiredVelocity.sqrMagnitude > Mathf.Epsilon )//make sure that desired velocity vector is not zero
		{
			if (!MixedMode ||
				(MixedMode && Mathf.Abs(angle)<110.0f && _animator.GetCurrentAnimatorStateInfo(0).IsName("BaseLayer.Locomotion") ))
			{
                //manually calculate rotation so as direct agent towards its velocity vector
